...
예를 들어 'Hello'와 'World'란 단어를 이용하여 클래스 이름을 만든다면 'HeelWorld'가 된다. 
낙타의 등처럼 오르락 내리락 한다고 하여 낙타표기법 카멜 표기법이라고 한다
```
### 메소드
- 메소드 선언 방법
``` sql
...
public final class Math extends Object 클래스의 정의

java 1.0 부터 있었던 클래스이다

staic double 타입인 E, staic double 타입인 PI 2개의 필드를 가진다

Math가 가지고 있는 메소드는 모두 static한 메소이다
Math 클래스는 필드와 메소드 모두 static 하기 때문에 인스턴스를 만들 필요가 없다
Math 클래스가 가지고 있는 모든 메소드는 static하기 때문에, API문서를 보면 Math. 후에 메소드를 사용한다는 것을 알 수 있다
```
- private 생성자
```
Math m = new Math(); 코드는 Math() 부분에서 컴파일 오류가 발생한다
Math()는 private 메소드라는 컴파일 에러가 발생한다

생성자를 정의 할 때 작성되어 있는 public은 어디서든 접근 가능하다는 의미이다
public을 private으로 변경하면, 해당 클래스 안에서는 접근 할 수 있게 된다
접근제한자가 private하다는 것은 인스턴스 생성을 못하게 한다는 의미이다

Math 클래스를 만든 사람은 여러 개 인스턴스를 생성하지 못하게 함으로써 메모리를 절약할 수 있도록 하고 Math.메소드() 형식으로 쉽게 호출하도록 만들었다

(인스턴스를 생성하지 못하도록 생성자를 private하게 만들었다)
```

> 자동생성 - 기본생성자는 생성자가 없을 경우 컴파일 할 때 자동으로 생성한다
